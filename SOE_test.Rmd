...
- <span style="font-size: 22px;"> 
From the firm’s perspective, the cost of labor has been rising faster than the increase in price of the products sold for the last four quarters
...
<center>
 <strong>Canada-U.S. Decomposition of Labour Productivity Growth, 1987-2021</strong>
</center>
</div>
...
<center>
<strong>Gross fixed capital formation by asset category (percentage of GDP, 2021)</strong>
</center>
</div>
...
.pull-left[
- <span style="font-size: 22px;"> 
The <span>green transition </span>will create challenges and opportunities for productivity growth. 
...
- <span style="font-size: 22px;"> 
Among G7 countries, Canada has the highest value added share of resource-dependent industries (see chart).

- <span style="font-size: 22px;"> 
Economic concentration in resource-dependant industries creates <strong>less investment opportunities in more complex manufacturing and services </strong>(see Annex E).
...
gchart <- highchart() %>%
  hc_chart(type = "column") %>%
  hc_title(text = "GDP share of Resource-Dependant Activities, 2000 vs 2019") %>%
...
lp_chart <- highchart() %>%
  ...
  hc_title(text = "Labour Productivity") %>%
  hc_xAxis(categories = c(" ")) %>%
  hc_yAxis(title = list(text = "USD, Median 3 year change (2018-21)"))%>%
  hc_legend(enabled = TRUE)%>%
    hc_add_theme(hc_theme_bloom()) %>%
    hc_yAxis(
    maxPadding = 0.1, # Adds 10% padding above the highest data point
...
rg_chart <- highchart() %>%
  hc_chart(type = 'bar') %>%
  hc_title(text = "Revenue Growth") %>%
  hc_xAxis(categories = c(" ")) %>%
  hc_yAxis(title = list(text = "Percentage, Median 3 year change (2018-21)")) %>%
  ...
  hc_legend(enabled = FALSE)%>%
    hc_add_theme(hc_theme_bloom())%>%
  hc_colors(colors = c("#AFABAB", "#A4B9CE"))%>%
    hc_yAxis(
    maxPadding = 0.1, # Adds 10% padding above the highest data point
    endOnTick = FALSE, # Prevents the axis from ending exactly on the last tick
    showLastLabel = TRUE, # Ensures the last label is displayed
...
eg_chart <- highchart() %>%
  ...
  hc_add_series(name = "Other Innovation Support", data = list(list(name = data$category[3], y = data$other[3])),
                type = 'bar', color = "#527FB6") %>%
  hc_title(text = "Employment Growth") %>%
  hc_xAxis(categories = c(" ")) %>%
  hc_yAxis(title = list(text = "Percentage, Median 3 year change (2018-21)"))%>%
   hc_plotOptions(
    series = list(
...
<div class="charts-container">
```{r,echo=FALSE,results='asis',fig.width=6, fig.height=3.05}
lp_chart
```
```{r,echo=FALSE,results='asis',fig.width=6, fig.height=3.05}
rg_chart
...
---
### There is no green transition without critical minerals
.pull-left[
- <span style="font-size: 22px;"> 
Minerals such as copper, nickel, lithium, graphite and cobalt are the building blocks of renewable energy projects and electric vehicles. 

- <span style="font-size: 22px;"> 
According to the World Bank, the current <strong>global production capacity of key minerals will need to increase by 500% by 2050 </strong>to meet the demands of the clean-energy transition.

- <span style="font-size: 22px;"> 
The current economic slowdown is creating major <strong>obstacles for critical mineral exploration, as less established prospectors, miners and exploration companies </strong>are particularly vulnerable to the whims of the market and struggle to raise funds. 

</span>
]
```{r,echo=FALSE, include=FALSE}
tsx <- read_excel("C:/Users/davreuxa/Documents/soe/tsx.xlsx")


tsx_graph <-highchart() %>%
  hc_chart(type = "line") %>%
  hc_title(text = "Equity Issuance on TSX Venture exchange (metals, and mining sector)") %>%
  hc_xAxis(categories = tsx$Year)%>%
  hc_yAxis(title = list(text = "$ billions")) %>%
  hc_add_series(name = "TSX", data = tsx$tsx) %>%
  hc_legend(enabled = FALSE) %>%
  hc_plotOptions(series = list(marker = list(enabled = TRUE))) %>%
  hc_tooltip(shared = FALSE, crosshairs = TRUE)%>%
  hc_colors(colors = c("grey")) %>%
  hc_add_theme(hc_theme_bloom()) %>%
  hc_caption(
    text = "Source: S&P Global Market Intelligence"
  ) 
  #hc_subtitle(text="Junior miner's financing remains highly volatile")
...
ev_chart <- highchart() %>%
  hc_chart(type = "column") %>%
  hc_title(text = "Electric Vehicle Sales") %>%
  hc_xAxis(categories = ev$quarter) %>%
  hc_yAxis_multiples(
    list(title = list(text = "Number of Vehicles (thousands)")),
    list(title = list(text = "Share of Sales (%)"), opposite = TRUE)
  ) %>%
...
---
### Efforts to build an AI ecosystem could unlock a productivity premium
.pull-left[
- <span style="font-size: 22px;"> 
Artificial Intelligence (AI) plays a significant role in the process of digitalization, transforming the way businesses and societies operate.

- <span style="font-size: 22px;"> 
A recent report from McKinsey estimates that automation trends could provide an annual productivity boost of <strong>0.2% to 3.3% </strong>from 2023 to 2040 and generative AI would contribute 0.1 to 0.6 percentage points of that growth. 

- <span style="font-size: 22px;"> 
A Goldman Sachs report is projects that global GDP could rise 7% over the next ten years on the back of a <strong>1.5 percentage point productivity gain from the adoption of generative AI.</strong>
