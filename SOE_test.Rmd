...
---
```{r cars, echo=FALSE, include = FALSE, warning = FALSE }
library(highcharter)

# Mock data similar to the one in the image uploaded
data <- data.frame(
...
colors <- c("#FF0000", "#FF0000", "#FF0000", "#FF0000", 
            "#008080", "#008080", 
...
test<-highchart() %>%
  ...
  hc_colors(colors) %>%
  hc_add_series(name = "Economic Indicators", data = data$values, dataLabels = list(enabled = TRUE)) %>%
  hc_tooltip(formatter = JS("function() { return this.x + ': <b>' + this.y + '%</b> (' + this.point.periods + ')'; }")) %>%
  hc_plotOptions(series = list(
    dataLabels = list(enabled = TRUE, format = '{point.y:.1f}% ({point.periods})')
  ))
...
```{r, echo=FALSE, out.width='100%', fig.align='center', class='centered-plot'}
test
```
...
---
###Canada’s latest growth outlook is further downgraded, dragged by consumption and housing
```{r, echo=FALSE, fig.align='center', fig.width=10, fig.height=6}
proj<-highchart() %>%
  hc_chart(type = "line") %>%
  ...
  hc_xAxis(categories = c(2022, 2023, 2024, 2025),
           plotBands = list(
             list(
               from = 2.0,  # Assuming the categories are zero-indexed
               to = 3.5,    # 2024 to 2025
               color = "rgba(204, 204, 204, 0.5)"  # Light gray background
             )
           )) %>%
  hc_yAxis(title = list(text = "GDP Growth (%)")) %>%
  hc_add_series(name = "Canada", data = c(3.4, 1.2, 0.9, 2.5)) %>%
  hc_add_series(name = "China", data = c(3.0, 5.1, 4.5, 4.4)) %>%
  hc_add_series(name = "Japan", data = c(1.0, 2.0, 0.8, 0.9)) %>%
  hc_add_series(name = "United States", data = c(1.9, 2.2, 0.8, 1.2)) %>%
  hc_add_series(name = "World", data = c(3.4, 2.9, 2.3, 2.6)) %>%
  hc_legend(enabled = TRUE) %>%
...
<div class="subpoints" style="position: absolute; top: 0; left: 0; width: 100%; padding: 10px; background-color: #FE5F55; text-align: center;">
 <span class="small-font"> Economic Dashboard</span> 
  <span class="small-font" style="margin-left: 100px;">Sector snapshot</span>
...
.pull-right[
```{r, echo=FALSE, fig.align='center', fig.width=7, fig.height=6, warning=FALSE}
debt1
```
