...
author: "Economic Research Analysis Team"
date: "January 2024"
output:
...
<strong> The Canadian economy is facing multiple headwinds pointing to slow economic activity </strong>in the first half of 2024.
...
.pull-left[
- <span style="font-size: 22px;"> 
Canada's economy <strong>shrank </strong>in the three months up to September 2023. 
...
monetary_chart <- highchart() %>%
  ...
  hc_caption(
    text = "Source: Statistics Canada, Table 36-10-0434-01; Gross domestic product (GDP) at basic prices, by industry, monthly (x 1,000,000); Bank of Canada, target for the overnight rate", style = list(fontSize = "9px")
  ) %>%
  ...
  hc_subtitle(text="GDP growth, monthly (left), compared to the Bank of Canada's policy interest rate (right)", style = list(fontSize = "18px"))
```
.pull-right[
...
proj<-highchart() %>%
  hc_chart(type = "line") %>%
  hc_subtitle(text = "Real GDP growth, past and projected, for Canada and the world (annualized growth rate) ", style = list(fontSize = "18px")) %>%
...
feddebt_graph <- highchart() %>%
  hc_chart(type = "line") %>%
  hc_subtitle(text = "Federal Debt-to-GDP ratio", style = list(fontSize = "16px")) %>%
...
```{r, echo=FALSE, fig.width=7, fig.height=5.80}
corpdebt <- read_excel("C:/Users/davreuxa/Documents/soe/corpdebt.xlsx")

corpdebt_chart <- highchart() %>%
  hc_chart(type = "column", zoomType = "xy") %>%
  hc_subtitle(text = "Total debt of non-financial corporations(left), and interest rate charges(right)", style = list(fontSize = "16px")) %>%
  ...
  hc_caption(
    text = "Source: Statistics Canada. Quarterly balance sheet, income statement, and selected financial ratios, by non-financial industries, non-seasonally adjusted (billions).", style = list(fontSize = "9px")
  ) %>%
...
.pull-left[
- <span style="font-size: 22px;"> 
<strong>High demand and a lack of supply </strong>in the housing market offset gains in household income.
...
- <span style="font-size: 22px;"> 
Canadian households allocating a higher portion of income to housing – on average, 55% towards total housing costs as of Q3 2023 – almost twice the rate compared to Q3 2019.

- <span style="font-size: 22px;"> 
Average household spending of ~15 cents of every after-tax dollar to service debt – the highest since 1990, which could limit consumer spending.
...
housecost_chart<- highchart() %>%
  ...
  hc_title(text = "Housing costs are taking an ever increasing share of households' income", style = list(fontSize = "20px")) %>%
  hc_subtitle(text="Housing affordability index, quarterly", style = list(fontSize = "18px"))%>%
  hc_colors(colors = c("grey", "#A4B9CE"))%>%
...
.pull-left[
- <span style="font-size: 22px;"> 
Canada's consumer price index rose by 3.4% in the year up to December, bringing the inflation cooldown to a halt.
...
foodinf_chart<- highchart() %>%
  ...
  hc_title(text = "Food inflation continues to decrease from its January 2023 peak, getting closer to the all-items inflation", style = list(fontSize = "20px")) %>%
  hc_colors(colors = c("grey", "#A4B9CE"))%>%
  hc_subtitle(text="Year-over-year inflation, monthly (%)", style = list(fontSize = "18px"))
...
const <- highchart() %>%
  ...
  hc_caption(
    text = "Source: StatsCan Table 36-10-0206-01 Indexes of business sector labour productivity, unit labour cost and related measures, seasonally adjusted.", style = list(fontSize = "9px")
  ) %>%
  hc_add_theme(hc_theme_bloom()) %>%
  hc_tooltip(
    pointFormat = '{series.name}: <b>{point.y:.2f}%</b><br/>',
  ...
  hc_subtitle(text = "Quarterly change in labor cost vs.  the implicit price deflator, %", style = list(fontSize = "16px"))%>%
  hc_title(text="The increase in cost of wages has outpaced the increase in prices of products sold for four consecutive quarters, before a recent uptick in prices.", style = list(fontSize = "20px"))%>%
  hc_colors(colors = c("grey", "#A4B9CE"))%>%
  hc_add_annotation( 
  labels = list(
    list(
...
.pull-right[
```{r, echo=FALSE, fig.align='center', fig.width=7.5, fig.height=6.5}
const
```
]
<div class="subpoints" style="position: absolute; top: 0; left: 0; width: 100%; padding: 10px; background-color: #A4B9CE; text-align: center;">
...
lp_chart<- highchart() %>%
  ...
  hc_subtitle(text = "Labour productivity in the business sector, Canada vs. US, annualized 4-year change (%)", style = list(fontSize = "18px")) %>%
  hc_title(text="Canada's labour productivity growth is now below pre-pandemic levels", style = list(fontSize = "20px"))%>%
...
```{r,echo=FALSE,results='asis',fig.width=7, fig.height=3.25}
...
skills_composition_chart
```
```{r,echo=FALSE,results='asis',fig.width=7, fig.height=3.5}
...
m_e_chart <- highchart() %>%
  hc_chart(type = "bar") %>%
  hc_subtitle(text = "M&E + IPP", style = list(fontSize = "16px")) %>%
...
non_res_chart <- highchart() %>%
  hc_chart(type = "bar") %>%
  hc_subtitle(text = "Non-residential Structures", style = list(fontSize = "16px")) %>%
...
gchart <- highchart() %>%
  ...
  hc_subtitle(text = "GDP share of Resource-Dependant Activities, 2000 vs 2019", style = list(fontSize = "16px")) %>%
...
ev_chart <- highchart() %>%
  hc_chart(type = "column") %>%
  hc_title(text="EV sales numbers have been soaring in the past two quarters", style = list(fontSize = "19px"))%>%
  hc_subtitle(text = "EV registrations (left) vs. share of total vehicle sales(right)", style = list(fontSize = "16px")) %>%
...
vacancy_graph <-highchart() %>%
 ...
 hc_add_series(name = "Unemployment-to-job Vacancies", data = vacancy$unemp, type = "line", yAxis = 0,
  ...
  hc_subtitle(text="Unemployment-to-job vacancy ratio (left) and job vacancies (right), monthly", style = list(fontSize = "16px"))
```
.pull-right[
